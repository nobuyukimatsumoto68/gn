...
struct WilsonGaussianAndDet2D {
  ...
  MC plaq( const Gauge& W, const Idx ix ) const {
    const Lattice::Coord x = lattice.get_coord(ix);
  ...
  MC staples( const Gauge& W, const Idx ix, const int mu ) const {
    ...
    const int nu = 1-mu;
    const Lattice::Coord x = lattice.get_coord(ix);
    const Lattice::Coord x_pmu = lattice.cshift(x, mu);
    const Lattice::Coord x_pnu = lattice.cshift(x, nu);
    const Lattice::Coord x_pmu_mnu = lattice.cshift(x_pmu, -nu-1);
    const Lattice::Coord x_mnu = lattice.cshift(x, -nu-1);
...
#pragma omp parallel for num_threads(nparallel)
    ...
    for(Idx ix=0; ix<lattice.vol; ix++) {
      ...
      tmp[ix] -= kappa * std::log( W(ix,0).Phi.determinant().real() );

      tmp[ix] += 0.5*lambda/Nc * ( W(ix,1).Phi - W(ix,1).id() ).squaredNorm();
      tmp[ix] -= kappa * std::log( W(ix,1).Phi.determinant().real() );
    }
  ...
  double dphi( const Gauge& W, const Idx ix, const int mu, const int a ) const {
    ...
    res *= lambda/Nc;
    res -= kappa * (W(ix,mu).Phi.inverse()*t[a]).trace().real();
    return res;
  }
  ...
  double dphi0( const Gauge& W, const Idx ix, const int mu ) const {
    ...
    res *= lambda/Nc;
    res -= kappa * W(ix,mu).Phi.inverse().trace().real();
    return res;
  }
